  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      {publications.length === 0 ? (
        <div className="rounded-2xl border border-[color:var(--border-default)] bg-[color:var(--bg-secondary)] px-6 py-16 text-center">
          <p className="text-sm text-[color:var(--text-secondary)]">
            {locale === "zh" ? "你还没有发布任何人格到市场" : "You haven't published any personas to the marketplace yet"}
          </p>
          <Link
            href={`/${locale}/personas`}
            className="mt-4 inline-block text-sm text-[color:var(--accent-warm)] transition-colors hover:text-[color:var(--accent-warm-hover)]"
          >
        ...
        <>
          <div className="mb-6 grid grid-cols-3 gap-4">
            <div className="rounded-xl border border-[color:var(--border-default)] bg-[color:var(--bg-secondary)] px-4 py-3">
              <p className="text-xs text-[color:var(--text-tertiary)]">{locale === "zh" ? "已发布" : "Published"}</p>
              <p className="mt-1 text-2xl font-semibold text-[color:var(--text-primary)]">{publications.length}</p>
            </div>
            <div className="rounded-xl border border-[color:var(--border-default)] bg-[color:var(--bg-secondary)] px-4 py-3">
              <p className="text-xs text-[color:var(--text-tertiary)]">{locale === "zh" ? "被收藏" : "Total Saves"}</p>
              <p className="mt-1 text-2xl font-semibold text-[color:var(--accent-warm)]">{totalSaves}</p>
            </div>
            <div className="rounded-xl border border-[color:var(--border-default)] bg-[color:var(--bg-secondary)] px-4 py-3">
              <p className="text-xs text-[color:var(--text-tertiary)]">{locale === "zh" ? "被使用" : "Total Uses"}</p>
              <p className="mt-1 text-2xl font-semibold text-[color:var(--accent-warm)]">{totalUsages}</p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            {publications.map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className="flex items-center gap-4 rounded-xl border border-[color:var(--border-default)] bg-[color:var(--bg-secondary)] px-5 py-4 transition-colors hover:border-[color:var(--border-hover)]"
                >
                  <PersonaAvatar avatar={p.identity.avatar} size={44} />
                  <div className="min-w-0 flex-1">
                    <p className="font-medium text-sm text-[color:var(--text-primary)] truncate">{localized.name}</p>
                    ...
                    {p.tags.length > 0 && (
                      <div className="mt-1.5 flex flex-wrap gap-1">
                        {p.tags.slice(0, 3).map((tag) => (
                          <span key={tag} className="rounded-full bg-[color:var(--bg-tertiary)] px-2 py-0.5 text-[10px] text-[color:var(--text-secondary)]">
                            {tag}
                          </span>
                        ))}
                      </div>
