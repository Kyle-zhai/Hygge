    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex h-14 shrink-0 items-center justify-between px-4">
        <Link href="/en" className="flex items-center gap-2.5">
          ...
          <span className="text-lg font-semibold italic text-[color:var(--text-primary)]">Hygge</span>
        </Link>
        <div className="flex items-center">
          <button
            onClick={() => setCollapsed(true)}
            className="hidden h-8 w-8 items-center justify-center rounded-lg text-[color:var(--text-tertiary)] transition-colors hover:bg-[color:var(--bg-tertiary)] hover:text-[color:var(--text-primary)] md:flex"
          >
            <PanelLeft className="h-4 w-4" />
          </button>
          <button
            onClick={() => setMobileOpen(false)}
            className="flex h-8 w-8 items-center justify-center rounded-lg text-[color:var(--text-tertiary)] transition-colors hover:bg-[color:var(--bg-tertiary)] hover:text-[color:var(--text-primary)] md:hidden"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
      ...
      {/* Divider */}
      <div className="shrink-0 mx-4 border-t border-[color:var(--bg-tertiary)]" />

      {/* Your Discussions */}
      <div className="px-3 pt-3 pb-3">
        <div className="mb-2 flex items-center justify-between px-3">
          <p className="text-xs font-medium text-[color:var(--text-tertiary)]">{locale === "zh" ? "你的讨论" : "Your Discussions"}</p>
          <div className="relative">
            <button
              ...
              className={`flex h-6 w-6 items-center justify-center rounded-md transition-colors ${
                typeFilter.size > 0
                  ? "text-[color:var(--accent-warm)] bg-[rgb(var(--accent-warm-rgb)/0.10)]"
                  : "text-[color:var(--text-tertiary)] hover:text-[color:var(--text-secondary)] hover:bg-[color:var(--bg-tertiary)]"
              }`}
            >
                      ...
                      return (
                        <button
                          ...
                          className={`flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-xs transition-colors ${
                            active
                              ? "bg-[rgb(var(--accent-warm-rgb)/0.10)] text-[color:var(--accent-warm)]"
        ...
        <div className="mb-2 px-1">
          <div className="flex items-center gap-2 rounded-lg border border-[color:var(--border-default)] bg-[color:var(--bg-primary)] px-2.5 py-1.5">
            ...
            <input
              ...
              className="w-full bg-transparent text-xs text-[color:var(--text-primary)] placeholder:text-[color:var(--text-tertiary)] outline-none"
            />
          </div>
        </div>
        ...
        <div className="space-y-0.5">
          {filteredHistory.length === 0 && (
            <p className="px-3 py-4 text-center text-xs text-[color:var(--text-tertiary)]">
              {searchQuery
              ...
              <div key={item.id} className="group relative">
                ...
                >
                  {item.status === "processing" || item.status === "pending" ? (
                    <Loader2 className="h-3.5 w-3.5 shrink-0 animate-spin text-[color:var(--accent-warm)]" />
                  ) : (
                ...
                <button
                  ...
                  className={`absolute right-2 top-1/2 -translate-y-1/2 flex h-6 w-6 items-center justify-center rounded-md text-[color:var(--text-tertiary)] transition-all hover:bg-[color:var(--border-default)] hover:text-[color:var(--text-primary)] ${
      ...
      <div className="shrink-0 border-t border-[color:var(--bg-tertiary)] px-3 py-3">
        {userEmail && (
          <button
            onClick={() => setUserMenuOpen((v) => !v)}
            className="flex w-full items-center justify-between rounded-lg px-3 py-2 transition-colors hover:bg-[rgb(var(--bg-tertiary-rgb)/0.60)]"
          >
            <div className="flex items-center gap-2.5 min-w-0">
              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[color:var(--bg-tertiary)] text-xs font-medium text-[color:var(--accent-primary)]">
                {userEmail[0].toUpperCase()}
              </div>
              <p className="truncate text-sm text-[color:var(--text-primary)]">{userEmail.split("@")[0]}</p>
            </div>
            <LogOut className="h-3.5 w-3.5 shrink-0 text-[color:var(--text-tertiary)]" />
          </button>
        )}
      </div>
    </div>
  ...
  return (
    <>
      ...
      <button
        ...
        className="no-print fixed left-3 top-3 z-40 flex h-9 w-9 items-center justify-center rounded-lg bg-[rgb(var(--bg-primary-rgb)/0.80)] text-[color:var(--text-secondary)] backdrop-blur transition-colors hover:bg-[color:var(--bg-tertiary)] hover:text-[color:var(--text-primary)] md:hidden"
      >
      ...
      {collapsed && (
        <button
          ...
          className="no-print fixed left-3 top-3 z-40 hidden h-9 w-9 items-center justify-center rounded-lg bg-[rgb(var(--bg-primary-rgb)/0.80)] text-[color:var(--text-secondary)] backdrop-blur transition-colors hover:bg-[color:var(--bg-tertiary)] hover:text-[color:var(--text-primary)] md:flex"
        >
          <PanelLeft className="h-5 w-5" />
        </button>
      )}
      ...
      <aside
        className={`no-print fixed inset-y-0 left-0 z-30 hidden w-[260px] flex-col border-r border-[color:var(--bg-tertiary)] bg-[color:var(--bg-primary)] transition-transform duration-300 md:flex ${
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside
              ...
              className="no-print fixed inset-y-0 left-0 z-50 flex w-[260px] flex-col border-r border-[color:var(--bg-tertiary)] bg-[color:var(--bg-primary)] md:hidden"
            >
              {sidebarContent}
            </motion.aside>
          </>
          ...
          return (
            <>
              ...
              <motion.div
                ...
                className="fixed z-[60] w-48 rounded-xl border border-[color:var(--border-default)] bg-[color:var(--bg-secondary)] p-1.5 shadow-2xl shadow-black/50"
              >
                <button
                  onClick={() => handleShare(item)}
                  className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-[color:var(--text-primary)] transition-colors hover:bg-[color:var(--bg-tertiary)]"
                >
          ...
          <>
            ...
            >
              ...
              <div className="flex justify-end gap-3">
                <button
                  onClick={() => setConfirmDeleteId(null)}
                  className="rounded-lg border border-[color:var(--border-default)] bg-[color:var(--bg-tertiary)] px-4 py-2 text-sm text-[color:var(--text-primary)] transition-colors hover:bg-[color:var(--bg-hover)]"
                >
                  {locale === "zh" ? "取消" : "Cancel"}
                </button>
                <button
        ...
        {userMenuOpen && userEmail && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-[60px] left-[12px] z-[60] w-[236px] rounded-xl border border-[color:var(--border-default)] bg-[color:var(--bg-secondary)] shadow-2xl shadow-black/60"
            >
              {/* User info */}
              <div className="flex items-center gap-3 px-4 py-4">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[color:var(--bg-tertiary)] text-sm font-medium text-[color:var(--accent-primary)]">
                  {userEmail[0].toUpperCase()}
                </div>
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium text-[color:var(--text-primary)]">{userEmail.split("@")[0]}</p>
                  <p className="truncate text-xs text-[color:var(--text-tertiary)]">{userEmail}</p>
                </div>
              </div>

              <div className="mx-3 border-t border-[color:var(--border-default)]" />
              ...
              <div className="px-4 py-3">
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <span
                    className={
                      isBYOK
                        ? "rounded-md border border-[rgb(var(--accent-warm-rgb)/0.40)] bg-[rgb(var(--accent-warm-rgb)/0.10)] px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-[color:var(--accent-warm)]"
                        : "rounded-md bg-[color:var(--bg-tertiary)] px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-[color:var(--accent-warm)]"
                    }
                  >
                    {isBYOK ? "BYOK" : plan}
                  </span>
                ...
                {isBYOK ? (
                  <>
                    <div className="flex items-baseline gap-1.5">
                      <span className="text-lg font-semibold text-[color:var(--text-primary)]">∞</span>
                      <span className="text-xs text-[color:var(--text-tertiary)]">
                        {locale === "zh" ? "不限次数 · 全部功能解锁" : "Unlimited · all features unlocked"}
                      </span>
                    </div>
                    <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[color:var(--bg-tertiary)]">
                      <div className="h-full w-full rounded-full bg-gradient-to-r from-[color:var(--accent-warm)] to-[#8B7355]" />
                    </div>
                  </>
                ) : (
                  <>
                    <div className="flex items-baseline gap-1.5">
                      ...
                      <span className="text-xs text-[color:var(--text-tertiary)]">/ {evaluationsLimit}</span>
                    </div>
                    <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-[color:var(--bg-tertiary)]">
                      <div
                        className="h-full rounded-full bg-gradient-to-r from-[color:var(--accent-warm)] to-[#8B7355] transition-all duration-300"
              ...
              <div className="p-1.5">
                <Link
                  ...
                  className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-[color:var(--text-secondary)] transition-colors hover:bg-[color:var(--bg-tertiary)] hover:text-[color:var(--text-primary)]"
                >
              ...
              <div className="flex items-center justify-center gap-3 px-3 py-2 text-[10px] text-[color:var(--text-tertiary)]">
                <Link href={`/${locale}/legal/terms`} className="hover:text-[color:var(--text-secondary)]" onClick={() => setUserMenuOpen(false)}>
                  {locale === "zh" ? "条款" : "Terms"}
                </Link>
                <span>·</span>
                <Link href={`/${locale}/legal/privacy`} className="hover:text-[color:var(--text-secondary)]" onClick={() => setUserMenuOpen(false)}>
                  {locale === "zh" ? "隐私" : "Privacy"}
                </Link>
                <span>·</span>
                <Link href={`/${locale}/legal/cookies`} className="hover:text-[color:var(--text-secondary)]" onClick={() => setUserMenuOpen(false)}>
                  Cookies
                </Link>
              </div>
            </motion.div>
