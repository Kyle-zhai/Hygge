...
export function buildClassifyTopicSystem(replyLanguage: ReplyLanguage = "en"): string {
  return `You are a topic analyzer. Given a user's discussion topic, classify it and generate 4-6 evaluation dimensions tailored to THIS specific topic, not generic ones.
...
${replyLanguageDirective(replyLanguage)} The "description" field must be in the reply language; "label_en" and "label_zh" stay as their respective languages regardless. Keep proper nouns (brand names, personal names) in the user's original spelling.
...
${replyLanguageDirective(replyLanguage)} The "description" field must be in the reply language; "label_en" and "label_zh" stay as their respective languages regardless. Keep proper nouns in the user's original spelling.
